...
// ──────────────────────────────────────────────────────────
function NavWabi({ active, onChange, onCreate, onProjects, onProfile, queue, onQueueClick, accent, collapsed, dark }) {
  // The center pill carries three full tab slots: Home, Projects,
  // Alerts. All three swap the underlying tab (active = current tab),
  // so the white pill indicator can land on whichever is selected.
  const tabs = [
    { id: 'home',     label: 'Home',
    ...
    { id: 'notif',    label: 'Notification',
      icon: (a, c) => <Icon.Bell filled={a} color={c} size={22} hasDot={!a}/> },
  ];
  const slotW = 72;
